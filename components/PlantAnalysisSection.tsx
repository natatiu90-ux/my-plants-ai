...
function SpeciesLearningCard({ analysis, plant, onKnowSpecies, onAddPhoto }: { analysis?: PlantAnalysisRecord; plant: Plant; onKnowSpecies?: () => void; onAddPhoto?: () => void }) {
  ...
  return (
    <div className="min-w-0 rounded-[20px] bg-white/55 p-3">
      ...
      <p className="mt-1.5 text-sm font-semibold leading-5 text-[#5f594f]">
      ...
      {presentation.isCompleted ? <p className="mt-1.5 text-sm font-medium leading-5 text-[#756d62]">{t("plantAnalysis.userSpeciesHint")}</p> : null}
      <div className="mt-3 flex flex-wrap gap-2">
        {presentation.showKnowNameAction && onKnowSpecies ? (
          <button type="button" onClick={onKnowSpecies} className="min-h-10 rounded-[16px] bg-[#ddf2dc] px-3 text-sm font-bold text-[#2d7a4f]">
        ...
        {onAddPhoto ? (
          <button type="button" onClick={onAddPhoto} className="min-h-10 rounded-[16px] bg-white/80 px-3 text-sm font-bold text-[#5f594f]">
...
export function PlantAnalysisSection({
    ...
    return (
      <section className="mt-4 min-w-0 rounded-[28px] bg-[#fffaf3] p-4 shadow-soft">
        ...
        {recommendationRefreshState?.status === "error" ? (
          <div className="mx-1 mt-3 rounded-[18px] bg-[#fff0e6] p-3 text-sm font-bold leading-5 text-[#8a5b24]">
            <p>{recommendationRefreshState.error ?? t("plantAnalysis.refreshFailedInline")}</p>
            {onRetryRecommendationRefresh ? (
              <button type="button" onClick={onRetryRecommendationRefresh} className="mt-2 min-h-10 rounded-[15px] bg-white/80 px-3 text-sm font-extrabold text-[#6f4e20]">
                {t("common.tryAgain")}
              </button>
            ) : null}
          </div>
        ...
        {hasPendingBaselineQuestions && !conversationalState.question ? (
          <p className="mx-1 mt-3 rounded-[18px] bg-[#eef5e8] p-3 text-sm font-bold leading-5 text-[#4f6946]">{t("plantAnalysis.pendingBaselineQuestions")}</p>
        ) : null}

        <div className="mt-3 grid gap-2.5">
          {conversationalState.question ? (
            <div className="min-w-0 rounded-[22px] bg-[#eef5e8] p-3.5">
              <p className="text-xs font-bold uppercase text-[#6f8c62]">{t("plantAnalysis.conversationQuestion")}</p>
              <p className="mt-2 text-base font-bold leading-6 text-[#3f3b35] [overflow-wrap:anywhere]">{conversationalState.question.question}</p>
              <p className="mt-1.5 text-sm font-medium leading-5 text-[#5f7a54] [overflow-wrap:anywhere]">{conversationalState.question.reason}</p>
              <div className="mt-3">
                <AnswerChips
            ...
            <div className="min-w-0 rounded-[22px] bg-[#eef5e8] p-3.5">
              <p className="text-sm font-bold leading-5 text-[#355f3d]">✓ {completedFact.label}: {completedFact.value}</p>
              <p className="mt-1.5 text-sm font-medium leading-5 text-[#4f4940] [overflow-wrap:anywhere]">{completedFact.conclusion}</p>
            </div>
          ) : (
            <div className="min-w-0 rounded-[22px] bg-[#eef5e8] p-3.5">
              <p className="text-xs font-bold uppercase text-[#6f8c62]">{t("plantAnalysis.conversationNow")}</p>
              <p className="mt-1.5 text-sm font-bold leading-5 text-[#355f3d] [overflow-wrap:anywhere]">
          ...
          {planActions.length ? (
            <div className="min-w-0 rounded-[20px] bg-white/65 p-3">
              ...
              <ul className="mt-2 grid gap-1.5 text-sm font-medium leading-5 text-[#5f594f] [overflow-wrap:anywhere]">
          ...
          {conversationalState.concern ? (
            <div className="min-w-0 rounded-[20px] bg-white/55 p-3">
              ...
              <p className="mt-1.5 text-sm font-medium leading-5 text-[#4f4940] [overflow-wrap:anywhere]">{conversationalState.concern}</p>
            </div>
          ) : null}

          {conversationalState.caution ? (
            <div className="min-w-0 rounded-[20px] bg-[#fff8e8] p-3">
              <p className="text-sm font-semibold leading-5 text-[#8a6230] [overflow-wrap:anywhere]">{conversationalState.caution}</p>
            </div>
          ) : null}
          ...
          {conversationalState.guidedAction ? (
            <div className="min-w-0 rounded-[20px] bg-white/65 p-3">
              <p className="text-sm font-semibold leading-5 text-[#4f4940]">
              ...
              <button
                ...
                className="mt-3 min-h-11 rounded-[18px] bg-[#ddf2dc] px-4 text-sm font-bold text-[#2d7a4f]"
              >
              ...
              {openGuide === conversationalState.guidedAction.type ? (
                <ol className="mt-3 grid list-decimal gap-1.5 pl-5 text-sm font-medium leading-5 text-[#5f594f]">
          ...
          <details className="min-w-0 rounded-[20px] bg-white/45 p-3">
            ...
            <div className="mt-3 grid gap-2">
              <p className="rounded-[18px] bg-[#eef5e8] p-3 text-sm font-semibold leading-5 text-[#355f3d] [overflow-wrap:anywhere]">{view.keyTakeaway}</p>
              {view.meaningfulObservations.length ? (
                <ul className="grid gap-1.5 text-sm font-medium leading-5 text-[#5f594f] [overflow-wrap:anywhere]">
                  {view.meaningfulObservations.map((item) => (
                    <li key={item}>{item}</li>
                  ))}
                </ul>
              ) : null}
              {view.likelyExplanation ? <p className="text-sm font-medium leading-5 text-[#4f4940] [overflow-wrap:anywhere]">{view.likelyExplanation}</p> : null}
            </div>
          </details>
        </div>
      </section>
  ...
  return (
    <section className="mt-4 min-w-0 rounded-[28px] bg-[#fffaf3] p-4 shadow-soft">
      ...
      {recommendationRefreshState?.status === "error" ? (
        <div className="mx-1 mt-2 rounded-[16px] bg-[#fff0e6] p-3 text-sm font-bold leading-5 text-[#8a5b24]">
          <p>{recommendationRefreshState.error ?? t("plantAnalysis.refreshFailedInline")}</p>
          {onRetryRecommendationRefresh ? (
      ...
      <div className="mt-3 grid gap-2">
        <div className="min-w-0 rounded-[22px] bg-[#eef5e8] p-3">
          <p className="text-sm font-semibold leading-5 text-[#355f3d] [overflow-wrap:anywhere]">{view.keyTakeaway}</p>
        </div>
        ...
        {view.likelyExplanation ? (
          <div className="min-w-0 rounded-[20px] bg-white/65 p-3">
            ...
            <p className="mt-2 text-sm font-medium leading-5 text-[#4f4940] [overflow-wrap:anywhere]">{view.likelyExplanation}</p>
          </div>
        ) : null}
        ...
        {view.followUp ? (
          <div className="min-w-0 rounded-[20px] bg-white/65 p-3">
            <p className="text-sm font-bold leading-5 text-[#4f4940] [overflow-wrap:anywhere]">{view.followUp.question}</p>
            <p className="mt-1 text-xs font-medium leading-4 text-[#8a8378] [overflow-wrap:anywhere]">{view.followUp.reason}</p>
            <div className="mt-3">
              <AnswerChips
        ...
        {view.answerConclusions.length ? (
          <details className="min-w-0 rounded-[20px] bg-[#eef5e8] p-3">
            ...
            <ul className="mt-2 grid gap-1.5 text-sm font-medium leading-5 text-[#4f6946] [overflow-wrap:anywhere]">
        ...
        {view.density === "serious" && view.lowConfidenceHypotheses.length ? (
          <div className="min-w-0 rounded-[20px] bg-white/45 p-3">
            <button
              ...
              className="flex w-full items-center justify-between text-left text-sm font-bold text-[#7a7166]"
            >
            ...
            {showOtherCauses ? (
              <ul className="mt-2 grid gap-1.5 text-sm font-medium leading-5 text-[#7a7166] [overflow-wrap:anywhere]">
