...
export function PlantStatusSection({
  ...
  return (
    <section className="mt-4 min-w-0 rounded-[28px] bg-[#fffaf3] p-5 shadow-soft">
      {commonName ? <p className="text-[13px] font-medium italic leading-5 text-[#8a8378] [overflow-wrap:anywhere]">{commonName}</p> : null}
      {plant.scientificName ? <p className="mt-0.5 text-[13px] italic leading-5 text-[#9a9286] [overflow-wrap:anywhere]">{plant.scientificName}</p> : null}
      <div className="mt-3">
